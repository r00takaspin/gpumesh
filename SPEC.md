...
### 13.1 Модель развёртывания

Координатор — single-host сервис с SQLite на локальном диске. Для такого профиля оптимален простой VPS + Dokku, без оркестрации.

| Компонент | Выбор | Обоснование |
|---|---|---|
| **Хостинг** | Hetzner CX22 (2 vCPU, 4 GB RAM, 40 GB NVMe) | €4/мес, достаточно для тысяч WebSocket-соединений |
| **Деплой** | [Dokku](https://dokku.com/) | Heroku-подобный PaaS на своём VPS: `git push` → авто-билд Dockerfile → zero-downtime деплой |
| **Контейнеризация** | Docker, multi-stage сборка → distroless образ (~8 MB) | Минимальный размер, никакого мусора в рантайме |
| **HTTPS** | Caddy + Let's Encrypt (плагин Dokku) | Автоматический выпуск и обновление сертификатов, одна команда |
| **Хранилище** | SQLite на локальном NVMe, Dokku volume | Данные переживают редеплои; локальный диск → минимальная задержка |

### 13.2 Dockerfile

Multi-stage сборка: Go-билд на `golang:alpine`, рантайм на `gcr.io/distroless/static-debian12:nonroot`. CGO отключён для максимальной переносимости. См. `Dockerfile` в корне репозитория.
...
# 2. Установка Dokku (одна команда)
wget https://raw.githubusercontent.com/dokku/dokku/v0.34.0/bootstrap.sh
sudo DOKKU_TAG=v0.34.0 bash bootstrap.sh
# → открыть IP в браузере, завершить веб-настройку

# 3. Создать приложение
dokku apps:create gpumesh

# 4. Volume для SQLite (переживает редеплои)
sudo mkdir -p /var/lib/dokku/data/storage/gpumesh
dokku storage:mount gpumesh /var/lib/dokku/data/storage/gpumesh:/data

# 5. Переменные окружения
dokku config:set gpumesh \
...
# 7. Git remote + первый деплой
git remote add dokku dokku@<vps-ip>:gpumesh
git push dokku main
```

Деплой — zero-downtime: Dokku поднимает новый контейнер, ждёт healthcheck, переключает трафик, гасит старый.

### 13.4 CI/CD

| Событие | Действие | Инструмент |
|---|---|---|
| **Pull request** | Тесты + линтер | GitHub Actions (`ci.yml`): `go vet`, `go test -race`, `golangci-lint` |
| **Тег `v*`** | Релиз бинарников | goreleaser → GitHub Releases (`.goreleaser.yml`) |
| **Push в `main`** | Деплой координатора | Ручной `git push dokku main`. Автоматизация через GitHub Actions добавится позже, когда потребуется CI-билд образа перед деплоем |

### 13.5 Бэкап SQLite

После запуска координатора добавляется **Litestream** — непрерывная репликация WAL в S3-совместимое хранилище (Hetzner Object Storage, ~€5/ТБ):

```bash
# Запускается как sidecar-процесс на VPS, мониторит /data/gpumesh.db
litestream replicate /data/gpumesh.db s3://gpumesh-backups/gpumesh.db
```

Восстановление — одна команда. Бэкап с точностью до транзакции.

### 13.6 Масштабирование

| Стадия | Решение |
|---|---|
| **Сейчас (MVP)** | Один VPS. Dokku масштабирует процессы внутри хоста: `dokku ps:scale gpumesh 4` |
| **Упёрлись в CPU/memory одного VPS** | Вертикальное масштабирование: Hetzner CX32 (€8) → CX42 (€16). Этого достаточно для тысяч одновременных доноров |
| **Нужна геораспределённость** | Stateless WebSocket-прокси на отдельных VPS по миру под Kamal/Nomad. Координатор с SQLite остаётся на одном хосте — он лёгкий, узким местом не станет |
