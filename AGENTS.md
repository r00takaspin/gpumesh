...
**«Запусти тесты» = запусти ВСЁ:**

1. **Go-тесты:** `go test ./... -count=1 -v -timeout 60s`
2. **API-тесты (Cucumber):** `cd tests && npm test` (api + ui профили)
   - Требует координатор на `http://localhost:8080`
   - UI-тесты требуют Playwright (`npx playwright install chromium`)

Оба должны быть зелёные перед коммитом/деплоем.
